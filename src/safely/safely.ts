 ...
 *          or `ok: false` and branded error if rejected.
 */
export async function safelyAwait<K extends SafelyPromise, E = unknown>(
	promise: K,
...
export function safelyThrow<T = unknown>(error: Error | string): SafelyResultFailure<T> {
	if (typeof error === "string") {
		const e = new Error(error) as SafelyError<T>;
		return { ok: false, error: e }
	}
	return { ok: false, error: error as SafelyError<T> }
}

export function safelyReturn<T = unknown, E = unknown>(value: T): SafelyResult<T, E> {
	return {
		ok: true,
		value
	}
